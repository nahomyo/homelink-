    ...
    <main className="min-h-screen bg-[#f6f4ef] text-[#17231f]">
      ...
      <section className="mx-auto grid max-w-[1320px] gap-12 px-6 pb-14 pt-12 lg:grid-cols-[1.08fr_.92fr] lg:px-10 lg:pb-20 lg:pt-16">
        ...
        <div className="relative min-h-[390px] overflow-hidden rounded-[2rem] bg-[#d7e0d4] p-7 shadow-[0_24px_60px_rgba(45,67,52,.12)] lg:min-h-[480px]">
          ...
          <div className="relative flex h-full flex-col justify-between">
            ...
            <div className="flex items-end justify-between"><div><p className="text-xs font-semibold uppercase tracking-[.18em] text-[#63766d]">Member since</p><p className="mt-1 font-serif text-lg font-bold text-[#193f34]">2024</p></div><span className="rounded-full bg-[#193f34] px-4 py-2 text-xs font-semibold text-white">{t.peopleFirst}</span></div>
          </div>
        </div>
      </section>
      ...
      <section id="jobs" className="bg-[#193f34] px-6 py-14 text-white lg:px-10 lg:py-16">
        <div className="mx-auto max-w-[1320px]">
          <div className="flex flex-col justify-between gap-6 md:flex-row md:items-end"><div><p className="mb-3 text-sm font-semibold text-[#e89a75]">{role === "worker" ? t.opportunities : role === "broker" ? "BROKER NETWORK" : t.hire}</p><h2 className="font-serif text-4xl font-bold tracking-tight sm:text-5xl">{role === "worker" ? t.workTitle : role === "broker" ? "Connect trusted workers and employers." : t.hireTitle}</h2></div><button onClick={() => setShowAll(!showAll)} className="self-start rounded-full border border-white/25 px-5 py-2.5 text-sm font-semibold transition hover:bg-white/10 md:self-auto">{showAll ? t.less : t.allJobs} <span className="ml-2">→</span></button></div>
          <div className="mt-10 flex flex-col gap-3 rounded-2xl bg-[#285448] p-3"><div className="flex flex-col gap-4 sm:flex-row"><div className="flex flex-1 items-center gap-3 rounded-xl bg-white px-4 py-3 text-[#193f34]"><span className="text-lg">⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} className="w-full bg-transparent text-sm outline-none placeholder:text-[#8c9890]" placeholder={t.search} /></div><button className="rounded-xl bg-[#e28b66] px-6 py-3 text-sm font-bold text-white transition hover:bg-[#ee9b78]">{t.searchButton}</button></div><div className="flex flex-wrap gap-2"><select value={jobType} onChange={(event) => setJobType(event.target.value)} className="filter-select"><option>All types</option><option>Full-time</option><option>Part-time</option><option>Weekend</option><option>Live-in</option><option>Live-out</option></select><select value={jobSkill} onChange={(event) => setJobSkill(event.target.value)} className="filter-select"><option>All skills</option><option>Childcare</option><option>Cooking</option><option>Cleaning</option></select><span className="filter-note">Filter by work type and skill</span></div></div>
          <div className="mt-6 grid gap-4 lg:grid-cols-3">{visibleJobs.map((job) => <article key={job.title} className="group rounded-2xl bg-[#f8f6f0] p-5 text-[#193f34] transition hover:-translate-y-1"><div className="flex items-start justify-between"><div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-full bg-[#e4b8a0] text-xs font-bold text-[#754c3d]">{job.initials}</div><div><p className="text-sm font-bold">{job.title}</p><p className="mt-1 text-xs text-[#718078]">{job.location} · {job.posted}</p></div></div><button aria-label={`Save ${job.title}`} onClick={() => toggleSaved(job.title)} className="text-xl text-[#9ca9a0] transition hover:text-[#d97952]">{saved.includes(job.title) ? "♥" : "♡"}</button></div><div className="mt-5 flex items-center gap-2 text-xs font-semibold"><span className="rounded-full bg-[#e6eee4] px-3 py-1.5 text-[#42705c]">{job.skill}</span><span className="rounded-full bg-[#f0e6d9] px-3 py-1.5 text-[#9b6045]">{job.type}</span><span className="ml-auto text-[#b87843]">★ {job.rating}</span></div><div className="mt-5 flex items-end justify-between border-t border-[#e2e2d8] pt-4"><div><p className="text-xs text-[#87928b]">{t.pay}</p><p className="mt-1 font-serif text-xl font-bold">{job.pay}</p></div><div className="flex items-center gap-3"><button onClick={() => applyForJob(job.title)} disabled={appliedJobs.includes(job.title)} className="rounded-full bg-[#d97952] px-3 py-2 text-xs font-bold text-white disabled:bg-[#9db4a2]">{appliedJobs.includes(job.title) ? "Applied" : "Apply now"}</button><button className="text-sm font-bold text-[#cc6b47]">{t.view} →</button></div></div></article>)}</div>
          {visibleJobs.length === 0 && <p className="py-10 text-center text-[#b4c3ba]">{t.noResults}</p>}
        </div>
      </section>

      {role === "worker" && <section id="worker-space" className="mx-auto max-w-[1320px] px-6 py-14 lg:px-10 lg:py-20">
        <div className="mb-8 flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><p className="mb-3 text-sm font-semibold text-[#b45d3c]">WORKER WORKSPACE</p><h2 className="font-serif text-4xl font-bold text-[#193f34]">Your next opportunity, organized.</h2><p className="mt-3 max-w-2xl text-[#718078]">Complete your profile, track applications, and stay close to employers in one simple place.</p></div><div className="rounded-xl bg-[#eaf0e8] px-4 py-3 text-sm font-semibold text-[#42705c]">✓ No worker fees, ever</div></div>
        <div className="grid gap-6 lg:grid-cols-[.75fr_1.25fr]">
          <aside className="rounded-2xl bg-[#193f34] p-5 text-white shadow-lg"><div className="flex items-center gap-4 border-b border-white/15 pb-5"><div className="flex h-14 w-14 items-center justify-center rounded-full bg-[#d97952] text-lg font-bold">{profile.name ? profile.name.slice(0, 2).toUpperCase() : "AM"}</div><div><p className="font-bold">{profile.name || "Your worker profile"}</p><p className="mt-1 text-xs text-[#b5cabc]">{profileSaved ? "Profile verified for review" : "Complete your profile"}</p></div></div><div className="mt-5 h-2 overflow-hidden rounded-full bg-white/15"><div className={`h-full rounded-full bg-[#e28b66] ${profileSaved ? "w-full" : "w-2/5"}`} /></div><p className="mt-2 text-xs text-[#b5cabc]">{profileSaved ? "100% profile complete" : "40% profile complete"}</p><div className="mt-6 space-y-1"><button onClick={() => setWorkerTab("overview")} className={`workspace-nav ${workerTab === "overview" ? "selected" : ""}`}>▦ Overview <span>{appliedJobs.length}</span></button><button onClick={() => setWorkerTab("profile")} className={`workspace-nav ${workerTab === "profile" ? "selected" : ""}`}>◎ My profile</button><button onClick={() => setWorkerTab("applications")} className={`workspace-nav ${workerTab === "applications" ? "selected" : ""}`}>✓ Applications <span>{appliedJobs.length}</span></button><button onClick={() => setWorkerTab("messages")} className={`workspace-nav ${workerTab === "messages" ? "selected" : ""}`}>✉ Messages <span>2</span></button></div></aside>
          <div className="rounded-2xl border border-[#dfe3d9] bg-white p-6 shadow-[0_14px_40px_rgba(45,67,52,.06)]">
            {workerTab === "overview" && <div><div className="flex items-start justify-between"><div><p className="text-sm font-semibold text-[#b45d3c]">THIS WEEK</p><h3 className="mt-1 font-serif text-3xl font-bold text-[#193f34]">Keep moving forward.</h3></div><button onClick={() => setWorkerTab("profile")} className="rounded-full border border-[#d7ded4] px-4 py-2 text-xs font-bold text-[#193f34]">Complete profile</button></div><div className="mt-7 grid gap-3 sm:grid-cols-3"><div className="stat-card"><span>Saved jobs</span><strong>{saved.length}</strong></div><div className="stat-card"><span>Applications</span><strong>{appliedJobs.length}</strong></div><div className="stat-card"><span>Profile views</span><strong>12</strong></div></div><div className="mt-7 rounded-xl bg-[#f6f4ef] p-4"><div className="flex items-center justify-between"><div><p className="text-sm font-bold text-[#193f34]">Interview invitation</p><p className="mt-1 text-xs text-[#718078]">A household wants to meet you tomorrow at 10:00 AM.</p></div><span className="rounded-full bg-[#f0e6d9] px-3 py-1 text-xs font-bold text-[#9b6045]">New</span></div><div className="mt-4 flex gap-2"><button onClick={() => setInterviewScheduled(true)} className="rounded-full bg-[#193f34] px-4 py-2 text-xs font-bold text-white">{interviewScheduled ? "Interview scheduled" : "Schedule interview"}</button><button onClick={() => setWorkerTab("messages")} className="rounded-full border border-[#d7ded4] px-4 py-2 text-xs font-bold text-[#193f34]">Message employer</button></div></div></div>}
      ...
      {role === "household" && <section id="employer-space" className="mx-auto max-w-[1320px] px-6 py-14 lg:px-10 lg:py-20">
      ...
      {role === "broker" && <section id="broker-space" className="mx-auto max-w-[1320px] px-6 py-14 lg:px-10 lg:py-20">
      ...
      <footer id="stories" className="border-t border-[#dedfd7] px-6 py-8 lg:px-10"><div className="mx-auto flex max-w-[1320px] flex-col justify-between gap-4 text-sm text-[#718078] sm:flex-row"><span className="font-serif text-base font-bold text-[#193f34]">HomeLink</span><span>{t.footer}</span><span>© 2024 HomeLink · Addis Ababa</span></div></footer>
      ...
      {authBusy && <div className="fixed bottom-5 left-5 z-20 rounded-2xl bg-[#193f34] px-5 py-4 text-sm text-white shadow-xl">Processing account access...</div>}
      {authError && <div className="fixed bottom-5 left-5 z-20 max-w-sm rounded-2xl border border-[#e8b7a4] bg-[#fff7f2] px-5 py-4 text-sm text-[#8b4d35] shadow-xl"><p className="font-bold">Account access needs attention</p><p className="mt-1">{authError}</p></div>}
