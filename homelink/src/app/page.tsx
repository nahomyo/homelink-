...
const simpleHeroCopy = {
  en: { eyebrow: "Find work in Addis Ababa", hero: "Find a job that fits your life.", body: "Search by role, skill, or location. Apply when you find the right job." },
  am: { eyebrow: "በአዲስ አበባ ስራ ያግኙ", hero: "ከህይወትዎ ጋር የሚስማማ ስራ ያግኙ።", body: "በስራ ድርሻ፣ ችሎታ ወይም ቦታ ይፈልጉ። የሚመጥንዎትን ስራ ሲያገኙ ያመልክቱ።" },
} as const;
...
export default function Home() {
  ...
  const hero = simpleHeroCopy[language];
  ...
  return (
    <main className="min-h-screen bg-[#f6f4ef] text-[#17231f]">
      <div className="city-strip"><div className="mx-auto flex max-w-[1320px] items-center justify-between px-6 lg:px-10"><span>● Addis Ababa · Ethiopia</span><span>{language === "am" ? "ስራ ይፈልጉ ወይም ሰራተኛ ያግኙ" : "Find work or hire help"}</span></div></div>
      ...
      <section className="mx-auto grid max-w-[1320px] gap-12 px-6 pb-14 pt-12 lg:grid-cols-[1.08fr_.92fr] lg:px-10 lg:pb-20 lg:pt-16">
        <div className="flex flex-col justify-center">
          <div className="mb-6 flex items-center gap-2 text-sm font-semibold text-[#b45d3c]"><span className="h-2 w-2 rounded-full bg-[#d77b54]" />{hero.eyebrow}</div>
          <h1 className="max-w-2xl font-serif text-5xl font-bold leading-[.98] tracking-[-.04em] text-[#193f34] sm:text-7xl">{hero.hero}</h1>
          <p className="mt-7 max-w-xl text-lg leading-8 text-[#63706a]">{hero.body}</p>
          ...
          <div className="quick-search mt-7 max-w-xl">
            <div className="flex items-center gap-3"><span className="quick-search-icon">⌕</span><input aria-label={t.search} value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") document.getElementById("jobs-search")?.scrollIntoView({ behavior: "smooth", block: "start" }); }} className="min-w-0 flex-1 bg-transparent text-sm text-[#193f34] outline-none placeholder:text-[#7c8981]" placeholder={t.search} /><a href="#jobs-search" className="quick-search-action">{t.navWork} <span aria-hidden="true">→</span></a></div>
            <div className="mt-3 flex flex-wrap gap-2 text-xs text-[#718078]"><span className="quick-search-chip">{w.childcare}</span><span className="quick-search-chip">{w.cooking}</span><span className="quick-search-chip">{w.cleaning}</span><span className="ml-auto">{language === "am" ? "ለመጀመር ይፈልጉ" : "Start with a role or skill"}</span></div>
          </div>
      ...
      <section id="jobs" className="bg-[#193f34] px-6 py-14 text-white lg:px-10 lg:py-16">
        <div className="mx-auto max-w-[1320px]">
          <div className="flex flex-col justify-between gap-6 md:flex-row md:items-end"><div><p className="mb-3 text-sm font-semibold text-[#e89a75]">{role === "worker" ? (language === "am" ? "የሚገኙ ስራዎች" : "AVAILABLE JOBS") : role === "broker" ? w.brokerNetwork : t.hire}</p><h2 className="font-serif text-4xl font-bold tracking-tight sm:text-5xl">{role === "worker" ? t.workTitle : role === "broker" ? w.brokerTitle : t.hireTitle}</h2></div><button onClick={() => setShowAll(!showAll)} className="self-start rounded-full border border-white/25 px-5 py-2.5 text-sm font-semibold transition hover:bg-white/10 md:self-auto">{showAll ? t.less : t.allJobs} <span className="ml-2">→</span></button></div>
          <div id="jobs-search" className="mt-10 flex scroll-mt-6 flex-col gap-3 rounded-2xl bg-[#285448] p-3"><div className="flex flex-col gap-4 sm:flex-row"><div className="flex flex-1 items-center gap-3 rounded-xl bg-white px-4 py-3 text-[#193f34]"><span className="text-lg">⌕</span><input aria-label={t.search} value={query} onChange={(event) => setQuery(event.target.value)} className="w-full bg-transparent text-sm outline-none placeholder:text-[#8c9890]" placeholder={t.search} /><span className="hidden rounded-full bg-[#eaf0e8] px-2 py-1 text-[10px] font-bold text-[#42705c] sm:inline">⌘ K</span></div><button className="rounded-xl bg-[#e28b66] px-6 py-3 text-sm font-bold text-white transition hover:bg-[#ee9b78]">{t.searchButton}</button></div><div className="flex flex-wrap gap-2"><select value={jobType} onChange={(event) => setJobType(event.target.value)} className="filter-select"><option value="All types">{w.allTypes}</option><option value="Full-time">{w.fullTime}</option><option value="Part-time">{w.partTime}</option><option value="Weekend">{w.weekend}</option><option value="Live-in">{w.liveIn}</option><option value="Live-out">{w.liveOut}</option></select><select value={jobSkill} onChange={(event) => setJobSkill(event.target.value)} className="filter-select"><option value="All skills">{w.allSkills}</option><option value="Childcare">{w.childcare}</option><option value="Cooking">{w.cooking}</option><option value="Cleaning">{w.cleaning}</option></select><span className="filter-note">{w.filterNote}</span></div></div>
